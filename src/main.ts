...
(async () => {
    ...
    try {
        ...
        // this.$cinerino をセットする。Cognito認証に失敗しても自動でリトライするのでここでは気にしない
        await cinerinoPlugin.install(Vue, {
            authConfig: window.appEnv.authConfig,
            cognitoUser: window.appEnv.cognitoUser,
            CINERINO_API_ENDPOINT: window.appEnv.CINERINO_API_ENDPOINT,
        });
    } catch (e) {
